...
Projects created with `spec init --project` include a hidden `.specos/` scaffold. Runtime files such as `frontend/package.json`, `frontend/index.html`, `frontend/tsconfig.json`, `frontend/vite.config.ts`, `backend/requirements.txt`, and `.env.example` are compiled from that scaffold instead of being regenerated by the model. Extra frontend packages can be added through `Environment -> Frontend` in `app.spec`, and backend/data runtime packages can be extended through `Environment -> Backend` and `Environment -> Data`.
